<template>
  <div class="ix-step" :class="stepClass" @click="emitChange">
    ...
    <div class="ix-step-head">
        <div v-if="canShowPercent" class="ix-step-head-percent">
          <div class="ix-step-head-percent-right">
            <div class="ix-step-head-percent-circle" :style="{ transform: 'rotate(' + rightDeg + 'deg)' }"></div>
          </div>
          <div class="ix-step-head-percent-left">
            <div class="ix-step-head-percent-circle" :style="{ transform: 'rotate(' + leftDeg + 'deg)' }"></div>
          </div>
        </div>
        <slot v-else-if="hasDotSlot" name="progressDot" :test="1" :status="curStatus"></slot>
        <span v-else-if="isDot" class="ix-step-head-dot"></span>
        <span v-else-if="hasIconSlot" class="ix-step-head-icon">
          <slot name="icon"></slot>
        </span>
        <ix-icon v-else-if="iconName" class="ix-step-head-icon" :name="iconName" />
        <ix-icon v-else-if="isFinish" class="ix-step-head-icon" name="check" />
        <ix-icon v-else-if="isError" class="ix-step-head-icon" name="close" />
        <span v-else class="ix-step-head-text">{{ stepNumber }}</span>
    </div>
...
export default defineComponent({
  ...
  setup(props: StepProps, { slots }: SetupContext) {
    ...
    const hasDotSlot = computed(() => hasSlot(slots, 'progressDot'))
    ...
    const placement = computed(() => (isDot.value ? 'horizontal' : (hasDotSlot.value ? 'vertical' : stepsProps.placement)))
    ...
    const canShowPercent = computed(
      () => isNumber(percent.value) && isActive.value && !isDot.value && !hasIconSlot.value,
    )
